...
public class FontController implements AdapterView.OnItemSelectedListener {
    ...
    public FontController(MainActivity activity, EventCallback callback) {
        mActivity = activity;
        mCallback = callback;
    }
    ...
    public final ColorPaletteListener backColorPaletteListener = new ColorPaletteListener() {
        @Override
        public void applyColor(int color) {
    ...
    private void sendFontChange(String fontName) {
        try {
            ...
            if(mCallback != null)mCallback.queueEvent(new LOEvent(LOEvent.UNO_COMMAND, ".uno:CharFontName", json.toString()));
    ...
    private void sendFontSizeChange(String fontSize) {
        try {
            ...
            if(mCallback != null)mCallback.queueEvent(new LOEvent(LOEvent.UNO_COMMAND, ".uno:FontHeight", json.toString()));
    ...
    private void sendFontColorChange(int color, boolean keepAlpha){
        try {
            ...
            if(mCallback != null)mCallback.queueEvent(new LOEvent(LOEvent.UNO_COMMAND, ".uno:Color", json.toString()));
    ...
    private void sendFontBackColorChange(int color, boolean keepAlpha) {
        try {
            ...
            if(mActivity.getTileProvider().isSpreadsheet()){
                ...
                if(mCallback != null)mCallback.queueEvent(new LOEvent(LOEvent.UNO_COMMAND, ".uno:BackgroundColor", json.toString()));
            }else {
                json.put("CharBackColor", valueJson);
                if(mCallback != null)mCallback.queueEvent(new LOEvent(LOEvent.UNO_COMMAND, ".uno:CharBackColor", json.toString()));
            }
    ...
    public void parseJson(String json) {
        ...
        try {
            ...
            while (keys.hasNext()) {
                ...
                mFontList.add(key);
                JSONArray array = jObject2.optJSONArray(key);
                if(array != null) {
