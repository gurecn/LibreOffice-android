...
public class LOKitTileProvider implements TileProvider {
     ...
     */
    public LOKitTileProvider(MainActivity context, InvalidationHandler messageCallback, String input, EventCallback callback) {
        mContext = context;
        mCallback = callback;
    ...
    private void postLoad() {
        ...
        mDocument.setPart(0);
        setupDocumentFonts();
        TheApplication.getMainHandler().post(() -> mContext.getDocumentPartViewListAdapter().notifyDataSetChanged());
    }
    ...
    public void addPart(){
        ...
        if(mDocument.getDocumentType() == Document.DOCTYPE_SPREADSHEET){
            try{
                ...
                if(mCallback != null)mCallback.queueEvent(new LOEvent(LOEvent.UNO_COMMAND, ".uno:Insert", jsonObject.toString()));
            }catch (JSONException e) {
                e.printStackTrace();
            }
        } else if (mDocument.getDocumentType() == Document.DOCTYPE_PRESENTATION){
            if(mCallback != null)mCallback.queueEvent(new LOEvent(LOEvent.UNO_COMMAND, ".uno:InsertPage"));
        }
    ...
    public void renamePart(String partName) {
        try{
            ...
            if(isPresentation()){
                if(mCallback != null)mCallback.queueEvent(new LOEvent(LOEvent.UNO_COMMAND_NOTIFY, ".uno:RenamePage", parameter.toString(),true));
            }else {
                JSONObject index = new JSONObject();
                index.put("type","long");
                index.put("value", getCurrentPartNumber()+1);
                parameter.put("Index", index);
                if(mCallback != null)mCallback.queueEvent(new LOEvent(LOEvent.UNO_COMMAND_NOTIFY, ".uno:Name", parameter.toString(),true));
            }
        }catch (JSONException e){
            e.printStackTrace();
        }
    ...
    public void removePart() {
        try{
            ...
            if(isPresentation()){
                if(mCallback != null)mCallback.queueEvent(new LOEvent(LOEvent.UNO_COMMAND_NOTIFY, ".uno:DeletePage", true));
                return;
            }
            ...
            index.put("value", getCurrentPartNumber()+1);
            parameter.put("Index", index);
            if(mCallback != null)mCallback.queueEvent(new LOEvent(LOEvent.UNO_COMMAND_NOTIFY, ".uno:Remove", parameter.toString(),true));
        }catch (JSONException e){
            e.printStackTrace();
        }
    }
    ...
    public boolean saveDocumentAs(final String filePath, String format, boolean takeOwnership) {
        ...
        if (!mOffice.getError().isEmpty()){
            ok = true;
            TheApplication.getMainHandler().post(() -> mContext.showCustomStatusMessage(mContext.getString(R.string.unable_to_save)));
        } else {
            ok = false;
            if (format.equals("svg")) {
                TheApplication.getMainHandler().post(() -> mContext.startPresentation(newFilePath));
    ...
    public void onSwipeLeft() {
        if (getCurrentPartNumber() < getPartsCount()-1) {
            if(mCallback != null)mCallback.queueEvent(new LOEvent(LOEvent.CHANGE_PART, getCurrentPartNumber()+1));
        }
    }
